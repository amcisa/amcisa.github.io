      ...
      if (mysqli_num_rows($exist) == 0){
        ...
        foreach($keyList as $prize){
          ...
          for($i=0;$i<count($nomination);$i++){
            ...
            $sql = "SELECT Vote FROM `amscarresult2017` WHERE `Prize` = '".$prize."' and `Nominee` = '".$nominee."'";
            $results= $db->query($sql);
            $currentVote = $results->fetch_row();
            $currentVote = $vote + $currentVote[0];
            $sql = "UPDATE `amscarresult2017` SET Vote = ".$currentVote." WHERE `Prize` = '".$prize."' and `Nominee` = '".$nominee."'";
            //print_r($sql);
            $db->query($sql);  
          }
        }
